...
const AddCase = () => {
    ...
    const removeDocumentRow = (caseId, rowIndex) => {
      ...
      const updatedCases = fetchedCases.map((caseData) =>
        caseData.case_id === caseId
          ? {
              ...caseData,
              documentRows: caseData.documentRows.filter((_, index) => index !== rowIndex),
            }
          : caseData
      );
      setFetchedCases(updatedCases);
    ...
    const handleFileUpload = async (caseId, rowIndex, file) => {
      try {
          ...
          const response = await fetch("http://localhost:8081/api/uploaddocumentss", {
              method: "POST",
              body: formData,
          });
    ...
    const handleSubmits = async () => {
      ...
      fetchedCases.forEach((caseData) => {
        if (caseData.documentRows && caseData.documentRows.length > 0) {
          caseData.documentRows.forEach(async (row) => { // Use async keyword here
            ...
            try {
              ...
              const response = await fetch('http://localhost:8081/api/uploadothercase', { // Use provided endpoint
                method: 'POST',
                headers: {
                  'Content-Type': 'application/json',
                },
