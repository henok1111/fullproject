const FetchAllCasesInformation = async (req, res) => {
    try {
        ...
        c.FIRDate,
        c.registrationDate,
        u.*,  -- Select all columns from the users table for the assigned judge
        c.case_status,
        c.judge_decision,
        c.is_paid,
    ...
    LEFT JOIN users u ON c.assigned_judge = u.id -- Join with users table on assigned_judge
    ...
    GROUP BY
        ...
        c.FIRDate,
        c.registrationDate,
        u.id, -- Group by assigned judge ID
        c.case_status,
        c.judge_decision,
        c.is_paid;
    `;
