const FetchCasesByProsecutor = async (req, db, res) => {
  ...
  try {
    ...
    GROUP_CONCAT(DISTINCT cs.sub_type_name) AS sub_type_names, -- Aggregating sub_type_name
    JSON_ARRAYAGG(
        JSON_OBJECT(
            'id', pc_client.id,
...
WHERE 
    pc.prosecutor_id = ?  -- Filter based on prosecutor_id from petitioner_case_map
GROUP BY
    c.case_id,
    c.file_path,
    c.description,
