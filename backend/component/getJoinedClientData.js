  ...
  `)
    .then(([rows, fields]) => {
      ...
      const formattedData = rows.map((client) => {
        const references = client.references ? client.references.split(';').map((ref) => {
          const [reference_id, reference_name, reference_mobile] = ref.split(' ');
          return { id: reference_id !== 'null' ? reference_id : null, reference_name, reference_mobile };
        }) : [];
